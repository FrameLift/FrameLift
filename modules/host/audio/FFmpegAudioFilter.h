...
// graph (abuffer → alimiter or dynaudnorm chain → abuffersink) used for audio
...
public:
    ...
    // graphDesc (e.g. "alimiter=..." or "dynaudnorm=...,asoftclip=type=tanh,volume=..."). The abuffersink is
    ...
    bool Configure(
        int rate, const AVChannelLayout& layout, AVSampleFormat fmt, AVRational tb, const std::string& graphDesc
    );
