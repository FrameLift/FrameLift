...
// visuals to the required `chrome` property (a C++ WindowChrome) set as an initial property
// by QtAppWindow. The root fills the whole window at a high z; only the top bar strip and the
// thin edge grips carry MouseAreas, so the transparent interior lets clicks fall through to
// the video and plugin overlays beneath it.
